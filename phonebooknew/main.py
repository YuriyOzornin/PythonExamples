...
def inputContact(contInd):
    ...
    for fieldKey, fieldDesc in FIELDS.items():
        ...
        newVal = userInput(f"Введите {fieldDesc} контакта{promptExt}", \
                           lambda uInp: (uInp != "" or contact[fieldKey] != "") and (fieldKey != "phone" or isPhone(
                               uInp)))  # если поле было пустое (новый контакт), то пустой ввод недопустим, также проверка, если тел. то корректный формат
        if newVal != "" and newVal != contact[
            fieldKey]:  # если введено непустое значение (т.е. требуется изменение) и оно не равно прежнему
            if fieldKey == "name" and len(
                    findContacts(newVal)) > 0:  # если это поле имени (ключевое) и оно уже занято др. контактом
    ...
    match operNum:
        ...
        case 2:  # удаление контактов
            ...
            if bool(len(indList)):
                ...
                userPrint("Нет подходящих контактов для удаления.")
        ...
        case 5:  # экспорт контактов
            ...
            if bool(len(indList)):
                ...
                if fileName[fileName.rfind(
                    ".")] != "." + formatId: fileName += "." + formatId  # если не задано расширение, оно автоматически добавляется
                expList = EXP_FUNCS[formatId](indList)
                writeFile(expList, fileName)
            else:
                userPrint("Нет контактов для экспорта.")
        ...
        case 6:  # импорт контактов
            ...
            if len(impList) > 0:
                ...
                if formatId in [el[0] for el in IMP_FUNCS.items()]:
                    ...
                    userPrint("Неизвестный формат импорта.")
            else:
                userPrint("Не удалось загрузить данные из файла.")